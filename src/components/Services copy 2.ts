        ...
        return html`
            <div class="md:mt-14">
                ...
                <div class="grid grid-cols-2 gap-2 mx-auto w-[90%] mt-6">
                    ...
                    ${map(this.modalTitles, (title, index) => 
                        html`
                            <div id="${title.replace(' ', '-').replace(',', '').toLocaleLowerCase()}" class="services-div">
                                <img class="absolute w-full h-full z-[-10]" src="../../images/services/${title.replace(' ', '-').replace(',', '')}.png" alt="${title}" aria-hidden="true" />
                                <span class="z-20 w-[90%] text-white text-center font-jose-700 text-[13px] leading-[17px] ">${title}</span>
                                <div id="${title.replace(' ', '-').replace(',', '').toLocaleLowerCase()}-modal" 
                                    class="services-modal ${this.modalStyles[index]} ${classMap({'hidden': this.activeModal !== index, 'flex': this.activeModal == index})} 
                                    ${classMap({'opacity-0': this.activeModal !== index, 'opacity-100': this.activeModal == index})} flex-col justify-between items-center"
                                >
